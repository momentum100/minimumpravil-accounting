                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3">Команда</th>
                                    <th scope="col" class="px-6 py-3">Счета</th>
                            ...
                            <tbody>
                                @forelse ($users as $user)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td class="px-6 py-4">{{ $user->email ?: ('TG: ' . $user->telegram_id ?: 'Н/Д') }}</td>
                                        <td class="px-6 py-4">{{ $user->team?->name ?: 'Н/Д' }}</td>
                                        ...
                                        <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                            <a href="{{ route('admin.users.show', $user) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Просмотр</a>
                                            <a href="{{ route('admin.users.edit', $user) }}" class="font-medium text-green-600 dark:text-green-500 hover:underline">Редактировать</a>
                                ...
                                @empty
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <td colspan="7" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                            Пользователи не найдены{{ $roleFilter ? ' с ролью ' . ($roleFilter === 'buyer' ? 'Баер' : ($roleFilter === 'agency' ? 'Агентство' : ucfirst($roleFilter))) : '' }}.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
